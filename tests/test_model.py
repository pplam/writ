...
def test_start_is_refused_while_blocked():
    data = base_state()
    with pytest.raises(ForgeError, match="blocked by incomplete dependencies"):
...
def test_complete_requires_acceptances_passed():
    data = base_state()
    with pytest.raises(ForgeError, match="unmet acceptance criteria"):
...
def test_acceptance_index_is_validated():
    data = base_state()
    with pytest.raises(ForgeError, match="out of range"):
        model.set_acceptance(data, "M01-001", 5, "passed")


def test_unknown_ids_are_rejected():
    data = base_state()
    with pytest.raises(ForgeError, match="unknown task"):
        model.get_task(data, "nope")
    with pytest.raises(ForgeError, match="unknown task or milestone"):
...
def test_dependency_cycle_is_detected():
    ...
    with pytest.raises(ForgeError, match="dependency cycle"):
