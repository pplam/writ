...
def test_dry_run_prints_the_prompt_without_a_run(planned, forge, project):
    code, out, _ = forge("dispatch", "M01-001", "--dry-run")
    assert code == 0
...
def test_prompt_includes_design_excerpt_and_constraints(planned, forge):
    forge("edit-task", "M01-001", "--allow", "internal/store", "--forbid", "api/")
    _, out, _ = forge("dispatch", "M01-001", "--dry-run")
...
def test_successful_dispatch_records_a_run(planned, forge, project):
    code, out, _ = forge("dispatch", "M01-001", "--agent", ECHO)
    assert code == 0
...
def test_failing_agent_marks_the_task_failed(planned, forge, project):
    code, _, _ = forge("dispatch", "M01-001", "--agent", FAIL)
    assert code == 3
    data = state.load(project)
...
def test_missing_agent_is_reported_clearly(planned, forge, project):
    code, _, err = forge("dispatch", "M01-001", "--agent", "definitely-not-a-real-agent")
    assert code == 2
...
def test_dispatch_respects_the_dependency_gate(planned, forge):
    code, _, err = forge("dispatch", "M02-001", "--agent", ECHO)
    assert code == 2 and "blocked by incomplete dependencies" in err
    assert forge("dispatch", "M02-001", "--agent", ECHO, "--force")[0] == 0


def test_timeout_terminates_the_agent(planned, forge, project):
    code, _, _ = forge("dispatch", "M01-001", "--agent", SLEEP, "--timeout", "1")
    assert code == 124
    run = next(iter(state.load(project)["runs"].values()))
    assert run["exit_code"] == 124 and run["status"] == "failed"


def test_extra_args_after_separator_reach_the_agent(planned, forge, project):
    agent = f"{sys.executable} -c 'import sys; print(sys.argv[1:])'"
    forge("dispatch", "M01-001", "--agent", agent, "--", "--model", "sonnet")
    run = next(iter(state.load(project)["runs"].values()))
...
def test_detached_run_is_observable_while_it_works(planned, forge, project):
    code, out, _ = forge("dispatch", "M01-001", "--agent", SLEEP, "--detach")
    ...
    _, runs, _ = forge("--json", "runs", "--active")
    run_id = json.loads(runs)[0]["id"]

    assert forge("cancel", run_id)[0] == 0
    data = state.load(project)
    assert data["runs"][run_id]["status"] == "cancelled"
    assert data["tasks"]["M01-001"]["status"] == "planned"


def test_cancel_refuses_inactive_runs(planned, forge, project):
    forge("dispatch", "M01-001", "--agent", ECHO)
    run_id = next(iter(state.load(project)["runs"]))
    code, _, err = forge("cancel", run_id)
    assert code == 2 and "not active" in err


def test_logs_by_run_and_by_task(planned, forge, project):
    forge("dispatch", "M01-001", "--agent", ECHO)
    run_id = next(iter(state.load(project)["runs"]))
    code, out, _ = forge("logs", run_id)
    assert code == 0 and "Task M01-001" in out
    code, by_task, _ = forge("logs", "M01-001")
    assert code == 0 and by_task == out
    code, tailed, _ = forge("logs", run_id, "--tail", "2")
    assert code == 0 and len(tailed.strip().splitlines()) <= 2


def test_logs_for_task_without_runs(planned, forge):
    code, _, err = forge("logs", "M03-001")
    assert code == 2 and "no runs yet" in err


def test_run_show_and_runs_filter(planned, forge, project):
    forge("dispatch", "M01-001", "--agent", ECHO)
    run_id = next(iter(state.load(project)["runs"]))
    code, out, _ = forge("run", run_id)
    assert code == 0 and f"id: {run_id}" in out and "alive: no" in out
    _, filtered, _ = forge("--json", "runs", "--task", "M01-001")
    assert [r["id"] for r in json.loads(filtered)] == [run_id]
    _, empty, _ = forge("--json", "runs", "--task", "M03-001")
    assert json.loads(empty) == []


def test_reap_reconciles_a_dead_run(planned, forge, project):
    forge("dispatch", "M01-001", "--agent", ECHO)
    run_id = next(iter(state.load(project)["runs"]))
    ...
    code, out, _ = forge("reap")
    assert code == 0 and run_id in out
    data = state.load(project)
    assert data["runs"][run_id]["status"] == "interrupted"
    assert data["tasks"]["M01-001"]["status"] == "planned"
    assert forge("reap")[1].strip() == "no stale runs"


def test_watch_once_renders_a_frame(planned, forge):
    code, out, _ = forge("watch", "--once", "--no-clear")
    assert code == 0 and "forge watch" in out and "tasks 0/4" in out
